...
import { salvarTransacoes, carregarTransacoes } from './utils/supabaseStorage';
...
function App() {
  ...
  const [abaAtiva, setAbaAtiva] = useState<'transacoes' | 'historico' | 'parcelas'>('transacoes');
  ...
  const adicionarTransacao = async (transacao: Omit<Transacao, 'id' | 'createdAt'>) => {
    ...
    const resumoCalculado = calcularResumo(novasTransacoes);
    setResumo(resumoCalculado);
  ...
  return (
    <VisibilityProvider>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        <Dashboard resumo={resumo} onRendaChange={handleRendaChange} />
