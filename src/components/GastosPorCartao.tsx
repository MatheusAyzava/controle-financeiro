...
const coresCartoes: Record<Cartao, string> = {
  ...
  'sem_cartao': 'from-slate-500 to-slate-600',
};
...
export default function GastosPorCartao({ gastosPorCartao }: GastosPorCartaoProps) {
  ...
  const cartoesComGastos = Object.entries(gastosPorCartao)
    .filter(([cartao, valores]) => valores.total > 0 && cartao !== 'sem_cartao') // Filtrar sem_cartao para mostrar separado
    .sort(([_, a], [__, b]) => b.total - a.total) as [Cartao, { matheus: number; alessandra: number; outros: number; total: number }][];
  
  const gastosSemCartao = gastosPorCartao.sem_cartao;
  const temGastosSemCartao = gastosSemCartao && gastosSemCartao.total > 0;
  ...
  return (
    <div className="card bg-gradient-to-br from-white to-slate-50 border-2 border-slate-200 shadow-xl">
      ...
      {cartoesComGastos.length === 0 && !temGastosSemCartao ? (
        ...
        <>
          ...
          {temGastosSemCartao && (
            <div className={`mt-${cartoesComGastos.length > 0 ? '6' : '0'} p-4 bg-gradient-to-br from-slate-100 to-slate-200 rounded-xl border-2 border-slate-300`}>
              <div className="flex items-center gap-2 mb-4">
                <div className="bg-gradient-to-br from-slate-500 to-slate-600 p-2 rounded-lg shadow-md">
                  <CreditCard className="w-4 h-4 text-white" />
                </div>
                <h4 className="font-bold text-slate-800 text-lg">Sem Cartão (Dinheiro/PIX/Transferência)</h4>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between p-2 bg-primary-50 rounded-lg">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-primary-700">
                    {formatarMoeda(gastosSemCartao.matheus, !valoresVisiveis)}
                  </span>
                </div>
                <div className="flex items-center justify-between p-2 bg-purple-50 rounded-lg">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-purple-700">
                    {formatarMoeda(gastosSemCartao.alessandra, !valoresVisiveis)}
                  </span>
                </div>
                <div className="flex items-center justify-between p-2 bg-orange-50 rounded-lg">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-orange-700">
                    {formatarMoeda(gastosSemCartao.outros, !valoresVisiveis)}
                  </span>
                </div>
                <div className="flex items-center justify-between p-2 bg-slate-200 rounded-lg border border-slate-400 mt-2">
                  <span className="text-sm font-semibold text-slate-800">Total</span>
                  <span className="font-extrabold text-slate-900">
                    {formatarMoeda(gastosSemCartao.total, !valoresVisiveis)}
                  </span>
                </div>
              </div>
            </div>
